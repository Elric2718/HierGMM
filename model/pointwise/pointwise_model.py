...
@model("pointwise", MetaType.ModelBuilder)
class PointwiseModel(object):
    ...
    def _predict(self, features, model_output):
        ...
        return tf.estimator.EstimatorSpec(
            ...
            predictions=outputs,
            prediction_hooks=[hooks.GraphPrinterHook()]
        )
    ...
    def _build_model(self, features):
        ...
        exp_seq_item_features, exp_seq_behavior_features = self._encode_sequence(
            self._feature_config.exposure_feature_conf,
            input_embeddings,
            scope_name="exposure_seq_encoder"
        )

        ipv_seq_item_features, ipv_seq_behavior_features = self._encode_sequence(
        ...
        exp_cross_features = self._perform_attention(
            ...
            behavior_features=exp_seq_behavior_features,
        ...
        ipv_cross_features = self._perform_attention(
            ...
            behavior_features=ipv_seq_behavior_features,
    ...
    def _encode_sequence(self, domain_features, input_features, scope_name):
        ...
        with defaults.variable_scope(scope_name):
            # Collect item feature and behavior feature respectively
            item_features_list, behavior_features_list = [], []
            for feature_field in domain_features:  # type: FeatureField
                group_name = feature_field.param(_GROUP_PARAM)
                if group_name == _GROUP_ITEM:
                    item_features_list.append(input_features[feature_field.full_name])
                elif group_name == _GROUP_BEHAVIOR:
                    behavior_features_list.append(input_features[feature_field.full_name])

            item_features = tf.concat(item_features_list, axis=2)
            behavior_features = tf.concat(behavior_features_list, axis=2)

            if self._model_config.sequence_encoder == "rnn":
                encoder_func = PointwiseModel._do_encode_sequence_with_rnn
            elif self._model_config.sequence_encoder == "dnn":
                encoder_func = PointwiseModel._do_encode_sequence_with_dnn
            else:
                raise ValueError("Unknown contextual item embedding extractor:", self._model_config.sequence_encoder)

            for i in range(self._model_config.sequence_encoder_blocks):
                with tf.variable_scope("block_{}".format(i)):
                    with tf.variable_scope("item_encoder"):
                        item_features = encoder_func(
                            item_features,
                    ...
                    with tf.variable_scope("behavior_encoder"):
                        behavior_features = encoder_func(
                            behavior_features,
                            hidden_size=self._model_config.behavior_encoder_hidden_size,
                            max_length=domain_features.max_length
                        )

            return item_features, behavior_features,


    @staticmethod
    def _do_encode_sequence_with_rnn(input_features, hidden_size, max_length):
        cell = tf.nn.rnn_cell.GRUCell(num_units=hidden_size)
        rnn_input = tf.unstack(input_features, num=max_length, axis=1)
        rnn_output, _ = tf.nn.static_rnn(cell=cell, inputs=rnn_input, dtype=tf.float32)
        return tf.stack(rnn_output, axis=1)  # (?,100,32)
    ...
    def _perform_attention(self, candidate_features, item_features, behavior_features, seq_length, hidden_size, scope_name):
        """
        Compute cross feature between candidate and sequential features via attention mechanism
        """
        with defaults.variable_scope(scope_name):
            values = tf.concat([item_features, behavior_features], axis=2)
