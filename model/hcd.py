...
class HierCloudDevice():
	...
	def _get_column_index(self, columns):
		assert set(columns).issubset(set(self.arch_conf.model_config.all_fields)), "Some columns are not in the data fileds."
		return [self.arch_conf.model_config.all_fields.index(str(x)) for x in columns]
	...
	def _build_sup_models(self, config, init_checkpoint, global_checkpoints, log_every):
		...
		for group_id, group_name in enumerate(self.group_sets):
			checkpoint = init_checkpoint if group_id < self.n_groups else global_checkpoints[group_name]
			print(f"For {group_name}, the model dir  is {os.path.join(self.args.checkpoint_dir, group_name)}; the init_checkpoint is {checkpoint}.")			
			...
			self.estimators[self.group_sets[group_id]] = tf.estimator.Estimator(
				...
				model_dir=os.path.join(self.args.checkpoint_dir, group_name),
				config=config
	        )
	...
	def _train(self, file_path):
		# load data
		self.emb = self._get_embedding(file_path, self.emb_idx)
		self.labels = self._get_lables(file_path, self.label_idx)
		self._initialize_unsup_models()

		mode_counter = defaultdict(int)
		for EM_iter, self.step_mode in enumerate(Oself.training_algo_modes):	
		...
		for EM_iter, self.step_mode in enumerate(self.training_algo_modes):	
			...
			if self.step_mode.startswith("E-"):
				...
				if self.store_attributes:
					### store parameters
					pd.DataFrame(self.sample_weights).to_csv(os.path.join(self.args.output_path, f"sample_weights.csv"), sep = ",")
					pd.DataFrame(self.sample_weights_tilde).to_csv(os.path.join(self.args.output_path, f"sample_weights_tilde.csv"), sep = ",")
			if self.step_mode.startswith("M-"):
				print("=======" * 10 + "\n" + f"Take M step: {mode_counter[self.step_mode]}-{EM_iter}.\n" + "=======" * 10)
				# M step
				self._take_M_step(file_path)
			if self.step_mode.startswith("Eval"):
				...
				test_labels = self._get_lables(test_file, self.label_idx)
				# counter
				...
				if self.store_attributes:
					pd.DataFrame(assignments).to_csv(os.path.join(self.args.output_path, f"assignments_{infer_strategy}.csv"), sep = ",")
					pd.DataFrame(assign_weights).to_csv(os.path.join(self.args.output_path, f"weights_{infer_strategy}.csv"), sep = ",")
	...
	def _take_E_step(self, file_path):
		...
		self.sample_weights_tilde = np.reshape(self.pop_weights, newshape = [1, self.n_groups]) * \
								self._prob_matrix(file_path, self.emb, np.expand_dims(np.squeeze(self.labels), axis = 1), "E-unsup") + EPSILON # np.array [n x K]		
		...
		self.sample_weights = sample_multiplier * self._prob_matrix(file_path, self.emb, np.expand_dims(np.squeeze(self.labels), axis = 1), self.step_mode) + EPSILON # np.array [n x K]		
	...
	def _prob_matrix(self, file_path, emb, labels, prob_mode, debug = False):			
		if emb is None:
			emb_dat = self._get_embedding(file_path, self.emb_idx)
		else:
			emb_dat = emb
		n_samples = emb_dat.shape[0]
		d_e = emb_dat.shape[1]
		assert d_e == EMB_DIM, "Embedding dimension does not match."
		...
		if self.step_mode in ("M-sup", "M-both"):	
			for group_id in (gid for _ in range(self.args.n_sampling) for gid in range(self.n_groups)):	# improvable: should update self.sup_init_checkpoints if n_sampling > 1
				if self.algo not in ["global", "global-eval"]:
					...
					# partition csv file
					working_file = file_path.replace(".csv", f"_{self.algo}_{self.n_groups}_{self.group_sets[group_id]}.csv")
	...
	def partition_csv(input_path, output_path, row_ids): 			
		assert np.sum(row_ids[1:]-row_ids[:-1] < 0) == 0, "Row ids are not sorted in an ascending order."
		if os.path.exists(output_path):
			os.remove(output_path)
		row_counter=0
		with open(output_path, "a") as writter:
			with open(input_path, "r") as reader:
				for line_idx, line in enumerate(reader):
					if line_idx == row_ids[row_counter]:
						writter.write(line.replace("\n", ''))
						writter.write("\n")
						row_counter += 1
					if row_counter == len(row_ids):
						break
	...
	@staticmethod
	def _get_embedding(file_path, col_idx):
		n_samples = sum(1 for _ in open(file_path))
		emb = np.zeros([n_samples, EMB_DIM])
		with open(file_path, "r") as file:
			for line_idx, line in enumerate(file):
				tmp_emb = line.replace("\n", '').split(";")[col_idx]
				emb[line_idx] = np.array(tmp_emb.split(',')).astype(float)
		return emb


	@staticmethod
	def _get_lables(file_path, col_idx):
		return np.squeeze(pd.read_csv(file_path, sep = ";", header = None, usecols = [col_idx]).values.astype(int))
	...
	def _predict(self, file_path, assignments, group_id, model_id):
		if assignments is None:
			...
			# partition csv file; improvable: there might be conflicts when multiple files are run at the same time.
			working_file = file_path.replace(".csv", f"_{self.algo}_{self.n_groups}_{self.group_sets[group_id]}.csv")
			self.partition_csv(input_path=file_path, 
				output_path=working_file,
				row_ids = row_ids)
	...
	def _infer_cluster(self, file_path, strategy = "random"): #improvable		
		...
		if strategy == "random":
			...
			#import pdb; pdb.set_trace()
			for label in [0, 1]:				
				prob_matrix = self._prob_matrix(file_path, None, label, prob_mode) # np.array [n_test x K]		
			...
			# inference
			prob_matrix = self._prob_matrix(file_path, None, 0, prob_mode) # np.array [n_test x K]		
			...
			# inference
			prob_matrix = self._prob_matrix(file_path, None, labels, prob_mode) # np.array [n_test x K]		
			sample_weights = np.reshape(self.pop_weights, newshape = [1, self.n_groups]) * prob_matrix  + EPSILON# np.array [n_test x K]		
			sample_weights = sample_weights/np.sum(sample_weights, axis = 1, keepdims = True) # np.array [n_test x K]	
			...
			labels = self._get_lables(file_path, self.label_idx)
	...
	def _eval(self, file_path, assignments, group_id, model_id):
		...
		labels = self._get_lables(working_file, self.label_idx)
		scores = np.array([prediction['score'] for _, prediction in enumerate(self._predict(file_path, assignments, group_id, model_id))])
		result = {'loss': roc_auc_score(labels, scores, average = "micro")} # the same as "macro" for binary
