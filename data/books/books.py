...
pip install gdown
gdown 'https://drive.google.com/uc?id=1roQnVtWxVE1tbiXyabrotdZyUY7FA82W': only books without users
      'https://drive.google.com/uc?id=1zmylV7XW2dfQVCLeg1LbllfQtHD2KUon&export=download': book-user interactions
...
import pandas as pd
import numpy as np
from script.data import time_based_split_v5
    ...
    with gzip.open(file_name) as fin:
        for l in fin:
            ...
            if (head is not None) and (count > head):
                break
        print(f"There are {count} lines in total.")
    df = pd.DataFrame(data, columns=['user_id', 'item_id', 'rating', 'ts'])
    return df

data_path = 'books'
books_df = load_data(os.path.join(data_path, 'goodreads_interactions_comics_graphic.json.gz'), head=None)
...
time_based_split_v5(valid_books, data_path, 20)
